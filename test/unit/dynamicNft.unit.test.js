...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT test", () => {
          let dynamicSvgNft, deployer, minter
          ...
          describe("Constructor", () => {
              it("Sets proper values, encodes SVGs correctly", async () => {
          ...
          describe("Mint NFT", () => {
              it("Mints nft, updates the mapping, emits the event", async () => {
                  const txValue = "1000"
                  const tx = await dynamicSvgNft.mintNft(txValue)
                  await tx.wait(1)
                  await expect(tx).to.emit(dynamicSvgNft, "CreatedNFT")
                  const value = await dynamicSvgNft.s_tokenIdToHighValue("0")
                  assert.equal(value.toString(), txValue)
              })
              ...
              it("shifts the token uri to lower when the price doesn't surpass the highvalue", async function () {
                  ...
                  const lowHighValue = ethers.utils.parseEther("100")

                  const tx = await dynamicSvgNft.mintNft(highValue)
                  await tx.wait(1)

                  const dynamicSvgNftMinter = dynamicSvgNft.connect(minter)
                  const mintTx = await dynamicSvgNftMinter.mintNft(lowHighValue)
                  await mintTx.wait(1)

                  const newTokenURU = await dynamicSvgNft.tokenURI("1")
                  const tokenURI = await dynamicSvgNft.tokenURI("0")

                  assert.equal(tokenURI, lowTokenUri)
                  assert.equal(newTokenURU, highTokenUri)
              })
          })
      })
